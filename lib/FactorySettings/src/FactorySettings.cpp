...
void fillRandom(char * buffer, size_t offset, size_t length, size_t size) {
...
void FactorySettings::_initialize() {
  ...
  strlcpy(values.ssid, FACTORY_SETTINGS_NAME_PREFIX, WL_SSID_MAX_LENGTH + 1);
  fillRandom(values.ssid, sizeof(FACTORY_SETTINGS_NAME_PREFIX) - 1, FACTORY_SETTINGS_NAME_SUFFIX_LENGTH, WL_SSID_MAX_LENGTH + 1);
  fillRandom(values.password, 0, FACTORY_SETTINGS_PASSWORD_LENGTH, WL_WPA_KEY_MAX_LENGTH + 1);
